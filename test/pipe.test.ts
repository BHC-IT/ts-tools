...
const resultEffectWithEffect = pipeEffect(
	(x: number) => x + 1,
	(x: number) => (x === 0 ? Maybe.nothing : Maybe.just(x)),
	(x: number) => Maybe.just(x - 1)
)
const resultEffectAsync = pipeEffectAsync(
	async (x: number) => x + 1,
	async (x: number) => (x === 0 ? Maybe.nothing : Maybe.just(x)),
	async (x: number) => Maybe.just(x - 1)
)
const resultEffectAsyncT = pipeEffectAsync(
	async (x: number) => x + 1,
	async (x: number) =>
		x === 0 ? Throwable.thrown('test') : Throwable.resolved(x),
	async (x: number) => Maybe.just(x - 1)
)
...
describe('test pipe', function () {
	...
	it('resultEffectAsyncT(...) should return Maybe.nothing', async function () {
		const i: Throwable<number> = (await resultEffectAsyncT(
			-1
		)) as any as Throwable<number>
