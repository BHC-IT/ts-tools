...
describe('test Array', function() {
	...
	it('entries', function() {
		const arr = [2, 1, 2, 5];
		expect([...Array.entries(arr)]).to.eql([[0, 2], [1, 1], [2, 2], [3, 5]]);
	...
	it('flat', function() {
		const arr = [[2, 0], [1, 1], [2, 2], [5, 3]];
		expect(Array.flat(arr)).to.eql([2, 0, 1, 1, 2, 2, 5, 3]);
		expect(arr).to.eql([[2, 0], [1, 1], [2, 2], [5, 3]]);
	});

	it('flatMap', function() {
		const arr = [[2, 0], [1, 1], [2, 2], [5, 3]];
		expect(Array.flatMap(arr, (e) => [e[0] + 1, e[1] + 2])).to.eql([3, 2, 2, 3, 3, 4, 6, 5]);
		expect(arr).to.eql([[2, 0], [1, 1], [2, 2], [5, 3]]);
	});
