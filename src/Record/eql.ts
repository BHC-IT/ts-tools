 ...
 * @internal
 */
export const eqlArray = <T extends unknown[], U extends unknown[]>(
	arrLeft: T,
	arrRight: U
): boolean =>
	arrLeft.reduce(
		(s: boolean, e: unknown, i: number) => s && eql(e, arrRight[i]),
		true
	) as boolean
...
export const eqlObj = <T extends object>(objLeft: T, objRight: T): boolean =>
	(Object.keys(objLeft) as Array<keyof typeof objLeft>).reduce(
		(s: boolean, e: keyof typeof objLeft) =>
			s && eql(objLeft[e], objRight[e]),
		true
	)
...
export const eql = <T, U>(objLeft: T, objRight: U): boolean => {
	...
	if (typeof objLeft === typeof objRight && typeof objLeft !== 'object')
		return objLeft === (objRight as unknown as T)

	if (objLeft instanceof Array && objRight instanceof Array)
		return objLeft.length === (objRight as unknown[]).length
			? eqlArray(objLeft, objRight as unknown[])
			: false
