...
export class Either<A, B> extends Effect<B> {
	...
	public static fmap = <A, B, C>(f:(a: B) => C, a: Either<A, B>): Either<A, C> => isRight(a.record) ? Either.right(f(a.record[1])) : new Either<A, C>(a.record)
	public fmap = <C>(f:(a: B) => C): Either<A, C> => Either.fmap(f, this)

	public static bind = <A, B, C>(f:(a: B) => Either<A, C>, a: Either<A, B>): Either<A, C> => isRight(a.record) ? f(a.record[1]) : new Either<A, C>(a.record)
	public bind = <C>(f:(a: B) => Either<A, C>): Either<A, C> => Either.bind(f, this)

	public static lift =  <A, B, C>(f:(a: B) => C): ((a: Either<A, B>) => Either<A, C>) => (a: Either<A, B>) => Either.fmap(f, a)
	...
	public static right = <A, B>(a: B): Either<A, B> => new Either<A, B>([true, a])

	public static isLeft = <A, B>(a: Either<A, B>): boolean => !Either.isRight(a)
	public isLeft = (): boolean => !Either.isRight(this);

	public static isRight = <A, B>(a: Either<A, B>): boolean => isRight(a.record)
	public isRight = (): boolean => Either.isRight(this)

	public static fromLeft = <A, B>(d: A, a: Either<A, B>) : A => !isRight(a.record) ? a.record[1] : d
	public fromLeft = (d: A) : A => Either.fromLeft(d, this)

	public static fromRight = <A, B>(d: B, a: Either<A, B>) : B => isRight(a.record) ? a.record[1] : d
	public fromRight = (d: B) : B => Either.fromRight(d, this);

	public static either = <A, B, C>(f:(a: A) => C, g:(a: B) => C, a: Either<A, B>) : C => isRight(a.record) ? g(a.record[1]) : f(a.record[1])
	public either = <C>(f:(a: A) => C, g:(a: B) => C) : C => Either.either(f, g, this)
